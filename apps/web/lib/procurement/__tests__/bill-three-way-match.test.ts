...
describe("buildBillDraftLinesFromPo", () => {
  it("aggregates quantities and landed cost from selected GRNs", () => {
    const order = {
      id: "po-1",
      voucher_number: "PO-1",
      supplier_id: "sup-1",
    ...
    expect(aggregates.get("po-line-1")?.quantity_received).toBe(8);
    ...
    expect(draftLines[0]?.quantity_billed).toBe("8");

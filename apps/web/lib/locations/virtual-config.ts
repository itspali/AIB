...
export function buildLocationMetaPatch(input: BuildLocationMetaInput): Record<string, unknown> {
  ...
  if (input.presence_type === "VIRTUAL" && input.virtual_configuration) {
    ...
    merged.configuration_metadata = {
      ...existingConfiguration,
      ...virtualMetadata,
    };
  } else {
    const { virtual: _removed, ...rest } = existingConfiguration;
    merged.configuration_metadata = Object.keys(rest).length > 0 ? rest : undefined;
    if (!merged.configuration_metadata) {

...
export async function fetchEntityDetailById(
  ...
  const [{ data: entityData, error: entityError }, { data: contactData, error: contactError }] =
    await Promise.all([
      supabase
        .from("entities")
  ...
  if (entityError || contactError || !entityData) return null;

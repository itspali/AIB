...
import type { InvoiceDraftLine } from "@/lib/sales/invoices/draft-form";

type Props = {
  lines: InvoiceDraftLine[];
  disabled?: boolean;
  fillHeight?: boolean;
  onChange: (lines: InvoiceDraftLine[] | ((current: InvoiceDraftLine[]) => InvoiceDraftLine[])) => void;
  onResolveSku: (key: string, sku: string) => void;
};

const INVOICE_COLUMNS: DocumentLineColumn[] = [
...
export function InvoiceLineEntryTable({
  lines,
  disabled = false,
  fillHeight = false,
  onChange,
  ...
  const patchLine = useCallback(
    (key: string, patch: Partial<InvoiceDraftLine>) => {
      onChange((current) =>
  ...
  return (
    <DocumentLineEntrySection title="Invoice lines" fillHeight={fillHeight}>
      <DocumentLineEntryGrid
        lines={lines}
        ...
        renderCell={(column, line) => {
          ...
          if (column.id === "quantity_invoiced") {
            return (
              <DocumentLineCompactInput
                ...
                align="right"
                onChange={(event) =>
                  patchLine(line.key, { quantity_invoiced: event.target.value })
                }
              />
            );
          }

...
export function OrganizationLogoUploader({
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      const path = buildTenantLogoStoragePath(tenantId, extension);
      ...
      >
        {displayPreview ? (
          ...
          <img
            ...
            className="h-20 w-20 rounded-lg object-contain ring-2 ring-border/80"
          />
        ) : (
          <div className="flex h-20 w-20 items-center justify-center rounded-lg bg-muted ring-2 ring-border/80">
            <Upload className="h-6 w-6 text-muted-foreground" />
          </div>
        )}

        <div>
          <p className="text-sm font-medium">Corporate logo</p>

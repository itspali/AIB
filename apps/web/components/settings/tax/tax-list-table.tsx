...
import { renderTaxListCell } from "@/components/settings/tax/tax-list-cells";
...
import { getTaxCellDisplayTexts } from "@/lib/tax/list-column-display-text";
...
type Props = {
  ...
  onSortChange: (field: TaxListSortField, direction: TaxListSortDirection) => void;
  onColumnWidthChange?: (columnId: TaxListColumnId, width: number | null) => void;
  selectedId: string | null;
  onSelect: (row: TaxCodeRow) => void;
};

export function TaxListTable({
  rows,
  columnPrefs,
  sortField,
  sortDirection,
  ...
  const resolveAutoWidth = useCallback(
    (columnId: TaxListColumnId, index: number) => {
      const column = getTaxColumnDef(columnId);
      return resolveListColumnAutoWidth({
        column,
        deviceClass,
        headerElement: frozen.headerRefs.current[index],
        bodyTexts: rows.flatMap((row) => getTaxCellDisplayTexts(columnId, row)),
        sortable: isSortableTaxColumn(columnId),
        measure: {
          ...measureHintsFromValueKind(column),
          mono: columnId === "code",
          tabular: columnId === "rate",
        },
      });
    },
  ...
  const resize = useResizableListColumns({
    ...
    deviceClass,
    getColumnDef: getTaxColumnDef,
    headerRefs: frozen.headerRefs,
    resolveAutoWidth,
  });

  const handleHeaderSort = (field: string) => {
    if (!isSortableTaxColumn(field)) return;
    const next = toggleTaxColumnSort(field, sortField, sortDirection);
    onSortChange(next.field, next.direction);
  };

  const lastDataColumnIndex = columns.length - 1;
                  ...
                  return (
                    <th
                      ...
                      className={cn(
                        ...
                        listTableHeaderCornerClass(index, lastDataColumnIndex)
                      )}
                  ...
                  >
                    {columns.map((columnId, index) => {
                      ...
                      return (
                        ...
                        >
                          {renderTaxListCell(columnId, row, {

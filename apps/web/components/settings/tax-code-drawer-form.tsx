...
type Props = {
  ...
  canEdit?: boolean;
  onClose: () => void;
  onAfterSave: (taxCodeId: string) => void;
  onEdit?: () => void;
  onDelete?: () => void;
};
...
function resolveDrawerTitle(surface: DrawerSurface, row: TaxCodeRow | null): string {
  if (surface === "create") return "New tax rule";
  if (surface === "edit") return "Edit tax rule";
  return row?.name ?? "Tax rule";
}

function TaxCodeMutateForm({
  row,
  isPending,
  onPatch,
  onSubmit,
  ...
  row: TaxCodeRow | null;
  isPending: boolean;
  form: TaxCodeFormValues;
  error: string | null;
  onPatch: (next: Partial<TaxCodeFormValues>) => void;
  onSubmit: () => void;
  onCancel: () => void;
}) {
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={form.kind}
            disabled={isPending}
            onValueChange={(value) => onPatch({ kind: value as TaxCodeKind })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="flex items-center justify-between gap-4 rounded-lg border border-border/80 border-black/[0.06] p-3 dark:border-white/10">
          ...
          <Switch
            ...
            disabled={isPending}
            onCheckedChange={(checked) => onPatch({ is_variable: checked })}
          />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isPending}
              onChange={(e) => onPatch({ effective_from: e.target.value || null })}
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isPending}
              onChange={(e) => onPatch({ effective_to: e.target.value || null })}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 rounded-lg border border-border/80 border-black/[0.06] p-3 dark:border-white/10">
          ...
          <Switch
            ...
            disabled={isPending}
            onCheckedChange={(checked) => onPatch({ is_inclusive_default: checked })}
          />
        </div>
        ...
        <div className="flex items-center justify-between gap-4 rounded-lg border border-border/80 border-black/[0.06] p-3 dark:border-white/10">
          ...
          <Switch
            ...
            disabled={isPending}
            onCheckedChange={(checked) => onPatch({ is_recoverable: checked })}
          />
        </div>
...
export function TaxCodeDrawerForm({
  open,
  surface,
  row = null,
  canEdit = false,
  ...
  const [form, setForm] = useState<TaxCodeFormValues>(toFormValues(editingRow));
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      ...
      toast.success(editingRow ? "Tax rule updated" : "Tax rule created");
      router.refresh();
      onAfterSave(result.taxCodeId);
    });
  };

  const drawerBody =
    surface === "peek" && row ? (
      <TaxPeekPanel row={row} canEdit={canEdit} onEdit={onEdit} onDelete={onDelete} />
    ) : isMutating ? (
      <TaxCodeMutateForm
        row={editingRow}
        form={form}

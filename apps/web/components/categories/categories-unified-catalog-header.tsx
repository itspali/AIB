...
import { CategoriesCatalogHeaderMobileMenu } from "@/components/categories/categories-catalog-header-mobile-menu";
import {
...
/** Categories catalog header — mirrors Items unified header layout. */
export function CategoriesUnifiedCatalogHeader({
  onNewCategory,
  count,
  controls,
  feedFilter,
  ...
  const searchField = feedFilter ? (
    <div className="relative min-w-0 flex-1">
      ...
      <Input
        ...
        aria-label="Filter category records"
        className={cn(
          LIST_TOOLBAR_CONTROL_HEIGHT,
          LIST_TOOLBAR_TEXT,
  ...
  const mobileToolbarMenu = (
    <CategoriesCatalogHeaderMobileMenu controls={controls} />
  );
  ...
  if (isMobileToolbar) {
    return (
      ...
      >
        <h1 className="shrink-0 text-base font-bold tracking-tight">Categories</h1>
  ...
  return (
    ...
    >
      <div className="items-unified-catalog-header__list-rail">
        <h1 className="shrink-0 text-lg font-bold tracking-tight md:text-xl">Categories</h1>
        <div className="shrink-0 text-sm tabular-nums text-muted-foreground">{count}</div>
        {searchField}
      </div>

-- Item tax supply category values for default_tax_category.
-- Migration: 20260601130000_item_tax_supply_category.sql

UPDATE public.items
SET default_tax_category = CASE upper(btrim(default_tax_category))
    WHEN 'STANDARD' THEN 'TAXABLE'
    WHEN 'REDUCED' THEN 'TAXABLE'
    WHEN 'ZERO_RATED' THEN 'TAXABLE'
    WHEN 'EXEMPT' THEN 'NON_TAXABLE'
    ELSE upper(btrim(default_tax_category))
END
WHERE upper(btrim(default_tax_category)) NOT IN (
    'TAXABLE', 'NON_TAXABLE', 'OUT_OF_SCOPE', 'NON_GST_SUPPLY'
);

CREATE OR REPLACE FUNCTION private.normalize_item_tax_supply_category(p_raw TEXT)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    v TEXT;
BEGIN
    v := upper(btrim(COALESCE(p_raw, 'TAXABLE')));
    IF v IN ('STANDARD', 'REDUCED', 'ZERO_RATED') THEN
        RETURN 'TAXABLE';
    END IF;
    IF v = 'EXEMPT' THEN
        RETURN 'NON_TAXABLE';
    END IF;
    IF v IN ('TAXABLE', 'NON_TAXABLE', 'OUT_OF_SCOPE', 'NON_GST_SUPPLY') THEN
        RETURN v;
    END IF;
    RETURN 'TAXABLE';
END;
...
-- Re-apply save_product_master_profile with normalized tax supply category.

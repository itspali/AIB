-- ====================================================================
-- PO per-document tax mode (inclusive / exclusive unit prices)
-- Migration: 20260617100000_purchase_order_prices_tax_inclusive.sql
-- ====================================================================

ALTER TABLE public.purchase_orders
    ADD COLUMN IF NOT EXISTS prices_tax_inclusive BOOLEAN NOT NULL DEFAULT FALSE;

-- Align existing POs with tenant procurement default where set
UPDATE public.purchase_orders po
SET prices_tax_inclusive = COALESCE(
    (
        SELECT (wcr.configuration_metadata ->> 'purchase_prices_tax_inclusive')::boolean
        FROM public.workspace_control_registry wcr
        WHERE wcr.tenant_id = po.tenant_id
          AND wcr.registry_key = 'PROCUREMENT_SETTINGS'
...
DROP FUNCTION IF EXISTS public.save_purchase_order(UUID, UUID, UUID, JSONB, UUID, INTEGER, JSONB, VARCHAR);

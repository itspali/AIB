-- ====================================================================
-- PO Phase 3 — editable line UOM + conversion factor on save
-- Migration: 20260616300000_purchase_order_line_uom.sql
-- ====================================================================

ALTER TABLE public.purchase_order_items
    ADD COLUMN IF NOT EXISTS uom_conversion_factor NUMERIC(15, 6) NOT NULL DEFAULT 1;

ALTER TABLE public.purchase_order_items
    DROP CONSTRAINT IF EXISTS purchase_order_items_uom_conversion_positive_chk;

ALTER TABLE public.purchase_order_items
    ADD CONSTRAINT purchase_order_items_uom_conversion_positive_chk
        CHECK (uom_conversion_factor > 0);

-- Backfill alternate-UOM lines from item master
UPDATE public.purchase_order_items poi
SET uom_conversion_factor = iu.conversion_factor
FROM public.item_uoms iu
INNER JOIN public.items i
    ON i.id = iu.item_id
   AND i.tenant_id = iu.tenant_id
WHERE poi.item_id = iu.item_id
  AND poi.tenant_id = iu.tenant_id
  AND poi.uom_code = iu.uom_code
  AND poi.uom_code IS DISTINCT FROM NULLIF(btrim(i.base_unit_of_measure), '');

-- --------------------------------------------------------------------
-- save_purchase_order — accept line uom_code; validate against item_uoms
-- --------------------------------------------------------------------

-- ====================================================================
-- AIB SMART ERP - ITEM TYPE / CLASSIFICATION CONSISTENCY GUARDS
-- Migration: 20260601120000_item_type_classification_guards.sql
-- ====================================================================
...
BEGIN
    ...
    IF p_classification = 'KIT_BUNDLE'::public.item_classification_type AND NOT COALESCE(p_is_bundle, FALSE) THEN
        RAISE EXCEPTION 'kit / bundle classification requires bundle flag';
    END IF;

    IF COALESCE(p_is_bundle, FALSE) AND p_item_type = 'PHYSICAL'::public.item_type
       AND p_classification <> 'KIT_BUNDLE'::public.item_classification_type THEN
        RAISE EXCEPTION 'bundled items must use kit / bundle classification';
    END IF;

    IF COALESCE(p_is_bundle, FALSE) AND p_item_type <> 'PHYSICAL'::public.item_type THEN
        RAISE EXCEPTION 'only goods items can be marked as a bundle';
    END IF;
END;
$$;

-- Patch save_product_master_profile: validate after item_type / classification / tracking resolve.

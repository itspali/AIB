-- Sync items.code with the product code field while only the style master row exists.
-- After variant SKU rows exist, code stays stable (matches UI lock once SKUs are generated).
...
DECLARE
    ...
    v_has_variant_sku_rows BOOLEAN;
BEGIN
    ...
    ELSE
        ...
        SELECT EXISTS (
            ...
            WHERE iv.item_id = p_item_id
              ...
              AND NOT iv.is_master
        )
        INTO v_has_variant_sku_rows;
        ...
        SET
            ...
            code = CASE
                WHEN v_has_variant_sku_rows THEN COALESCE(code, v_trimmed_sku)
                ELSE v_trimmed_sku
            END,

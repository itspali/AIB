-- PO transaction-level trade discount (GST-compliant header discount)

ALTER TABLE public.purchase_orders
    ADD COLUMN IF NOT EXISTS transaction_discount_percentage NUMERIC(5, 2) NOT NULL DEFAULT 0,
    ADD COLUMN IF NOT EXISTS transaction_discount_amount NUMERIC(15, 4) NOT NULL DEFAULT 0,
    ADD COLUMN IF NOT EXISTS transaction_discount_type TEXT NOT NULL DEFAULT 'percent';

ALTER TABLE public.purchase_orders
    DROP CONSTRAINT IF EXISTS purchase_orders_transaction_discount_percentage_chk;

ALTER TABLE public.purchase_orders
    ADD CONSTRAINT purchase_orders_transaction_discount_percentage_chk
        CHECK (transaction_discount_percentage >= 0 AND transaction_discount_percentage <= 100);

ALTER TABLE public.purchase_orders
    DROP CONSTRAINT IF EXISTS purchase_orders_transaction_discount_amount_chk;

ALTER TABLE public.purchase_orders
    ADD CONSTRAINT purchase_orders_transaction_discount_amount_chk
        CHECK (transaction_discount_amount >= 0);

ALTER TABLE public.purchase_orders
    DROP CONSTRAINT IF EXISTS purchase_orders_transaction_discount_type_chk;

ALTER TABLE public.purchase_orders
    ADD CONSTRAINT purchase_orders_transaction_discount_type_chk
        CHECK (transaction_discount_type IN ('percent', 'amount'));

-- Gatekeeper: zero transaction discount when policy is off
CREATE OR REPLACE FUNCTION public.purchase_order_transaction_discount_gatekeeper()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
    IF NOT private.get_procurement_control_flag(NEW.tenant_id, 'allow_transaction_discounts') THEN
        NEW.transaction_discount_percentage := 0.00;
        NEW.transaction_discount_amount := 0.0000;
        NEW.transaction_discount_type := 'percent';
    END IF;
...
DROP TRIGGER IF EXISTS purchase_orders_transaction_discount_gatekeeper
    ON public.purchase_orders;

CREATE TRIGGER purchase_orders_transaction_discount_gatekeeper
    BEFORE INSERT OR UPDATE ON public.purchase_orders
    FOR EACH ROW
    EXECUTE FUNCTION public.purchase_order_transaction_discount_gatekeeper();

-- ====================================================================
-- GST PO import tax: resolver wiring, zero vendor GST on imports
-- Migration: 20260618130000_gst_po_import_tax.sql
-- ====================================================================
...
AS $$
    SELECT CASE upper(trim(COALESCE(p_supply_nature, '')))
        WHEN 'IMPORT_GOODS' THEN FALSE
        WHEN 'IMPORT_SERVICES' THEN FALSE
...
CREATE OR REPLACE FUNCTION private.resolve_line_tax(
    ...
    p_tax_mechanism public.gst_tax_mechanism DEFAULT 'FORWARD'
)
RETURNS TABLE (
    tax_code_id     UUID,
    rate            NUMERIC,
...
BEGIN
    ...
    v_zero_vendor := p_tax_mechanism IN (
        'IMPORT_IGST'::public.gst_tax_mechanism,
        'REVERSE_CHARGE'::public.gst_tax_mechanism,
        'ZERO_RATED'::public.gst_tax_mechanism
    );

    v_qty := GREATEST(COALESCE(p_qty, 0), 0);
    v_net_unit := GREATEST(COALESCE(p_unit_price, 0) - COALESCE(p_discount_per_unit, 0), 0);

    IF NOT v_zero_vendor THEN
    ...
    IF v_zero_vendor THEN
        v_tax_code_id := NULL;
        v_rate := 0;
        v_is_variable := FALSE;
    ELSIF v_tax_code_id IS NOT NULL AND v_is_variable THEN
        v_rate := 0;
        FOR v_rule IN
    ...
    IF v_zero_vendor OR COALESCE(v_rate, 0) = 0 THEN
        v_base := v_gross;
        v_tax := 0;
        v_components := '[]'::jsonb;
        ...
        IF jsonb_array_length(v_components) = 0 THEN
            IF upper(trim(COALESCE(p_tax_supply_nature, ''))) = 'INTRASTATE' THEN
                ...
                v_components := jsonb_build_array(
                    jsonb_build_object('name', 'CGST', 'rate', ROUND(v_rate / 2, 4), 'amount', v_cgst_amount),
                    jsonb_build_object('name', 'SGST', 'rate', ROUND(v_rate / 2, 4), 'amount', ROUND(v_tax - v_cgst_amount, 4))
                );
            ELSE
                v_components := jsonb_build_array(
                    jsonb_build_object('name', 'IGST', 'rate', v_rate, 'amount', v_tax)
                );
            END IF;
        ELSE
            ...
            IF v_drift <> 0 AND v_last_index >= 0 THEN
                v_last_amount := COALESCE((v_components -> v_last_index ->> 'amount')::NUMERIC, 0);
...
RETURNS TABLE (
    tax_code_id UUID, rate NUMERIC, is_inclusive BOOLEAN,
    taxable_base NUMERIC, tax_amount NUMERIC, line_total NUMERIC, tax_components JSONB
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, private
AS $$
    SELECT * FROM private.resolve_line_tax(
        p_item_id, p_qty, p_unit_price, p_discount_per_unit,
        p_price_tax_inclusive, p_tax_supply_nature, p_tax_mechanism
    );
$$;

REVOKE ALL ON FUNCTION public.resolve_line_tax(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, TEXT, public.gst_tax_mechanism) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_line_tax(UUID, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, TEXT, public.gst_tax_mechanism) TO authenticated;

-- save_purchase_order: wire GST supply context
...
BEGIN
    ...
    IF NOT private.can_edit_purchase_orders() THEN RAISE EXCEPTION 'purchase order edit permission required'; END IF;
    ...
    IF v_supplier.type NOT IN ('SUPPLIER', 'MUTUAL_PARTNER') THEN RAISE EXCEPTION 'entity is not a supplier'; END IF;
    ...
    v_allow_issued_edit := private.get_procurement_control_flag(v_tenant_id, 'allow_edit_issued_purchase_orders');
    v_allow_discounts := private.get_procurement_control_flag(v_tenant_id, 'allow_line_item_discounts');

    IF p_prices_tax_inclusive IS NOT NULL THEN
        v_purchase_tax_inclusive := p_prices_tax_inclusive;
    ELSE
        v_purchase_tax_inclusive := private.get_procurement_control_flag(v_tenant_id, 'purchase_prices_tax_inclusive');
    END IF;

    IF p_purchase_order_id IS NOT NULL THEN
        SELECT id, document_status, destination_location_id
        INTO v_po_id, v_po_status, v_existing_destination_id
        FROM public.purchase_orders WHERE id = p_purchase_order_id AND tenant_id = v_tenant_id;
    ...
    ELSE
        v_voucher_number := public.generate_next_voucher_string(
            v_tenant_id, 'PURCHASE_ORDER'::public.document_voucher_type, NULL, p_destination_location_id
        );
        INSERT INTO public.purchase_orders (
            tenant_id, destination_location_id, supplier_id, voucher_number,
            document_status, payment_terms_days, currency_code, custom_fields,
            prices_tax_inclusive, tax_supply_nature, tax_mechanism,
            supplier_tax_treatment, supplier_country_code, incoterms_code, rcm_applicable, created_by
        ) VALUES (
            v_tenant_id, p_destination_location_id, p_supplier_id, v_voucher_number,
            'DRAFT'::public.purchase_document_status, v_payment_terms, v_currency_code, v_custom_fields,
            v_purchase_tax_inclusive, v_tax_supply_nature, v_tax_mechanism,
    ...
    LOOP
        ...
        IF v_qty IS NULL OR v_qty <= 0 THEN RAISE EXCEPTION 'quantity_ordered must be greater than zero'; END IF;
        IF v_unit_price < 0 THEN RAISE EXCEPTION 'unit_price_contractual cannot be negative'; END IF;
        IF NOT v_allow_discounts THEN v_discount_pct := 0; v_discount_amt := 0; END IF;
        IF v_discount_pct > 100 THEN RAISE EXCEPTION 'discount_percentage cannot exceed 100'; END IF;
        ...
        IF NOT COALESCE(v_track_inventory, FALSE) THEN RAISE EXCEPTION 'variant does not track inventory'; END IF;

        v_base_uom := COALESCE(NULLIF(btrim(v_base_uom), ''), 'PCS');
        IF v_requested_uom IS NULL OR v_requested_uom = v_base_uom THEN
            v_line_uom := v_base_uom; v_uom_conversion := 1;
        ELSE
            SELECT iu.conversion_factor INTO v_uom_conversion
            FROM public.item_uoms iu
            WHERE iu.item_id = v_item_id AND iu.tenant_id = v_tenant_id AND iu.uom_code = v_requested_uom LIMIT 1;
        ...
        IF v_discount_amt > 0 THEN
            v_line_discount := LEAST(v_discount_amt, v_line_extension); v_discount_pct := 0;
        ELSIF v_discount_pct > 0 THEN
            v_line_discount := LEAST(v_line_extension, v_line_extension * v_discount_pct / 100); v_discount_amt := 0;
        ELSE v_line_discount := 0; END IF;

        v_discount_per_unit := CASE WHEN v_qty > 0 THEN ROUND(v_line_discount / v_qty, 4) ELSE 0 END;
        ...
        FROM private.resolve_line_tax(
            v_item_id, v_qty, v_unit_price, v_discount_per_unit,
            v_purchase_tax_inclusive, v_tax_supply_nature, v_tax_mechanism
        ) LIMIT 1;
        ...
        INSERT INTO public.purchase_order_items (
            tenant_id, purchase_order_id, item_id, variant_id, uom_code, uom_conversion_factor,
            quantity_ordered, unit_price_contractual, discount_percentage, discount_amount,
            tax_rate_percentage, line_tax_amount, line_total_gross, tax_components_json
        ) VALUES (
            v_tenant_id, v_po_id, v_item_id, v_variant_id, v_line_uom, v_uom_conversion,
            v_qty, v_unit_price, v_discount_pct, v_discount_amt,
            v_tax_rate, v_line_tax, v_line_gross, v_tax_components
        );
        v_line_count := v_line_count + 1;
    END LOOP;

    IF v_line_count = 0 THEN RAISE EXCEPTION 'no valid purchase order lines were saved'; END IF;

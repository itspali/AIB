-- Reconcile post_goods_receipt with QC/exception workflow + reject disposition + partial QC
-- Migration: 20260622100000_reconcile_post_goods_receipt_qc_exception.sql

DO $$ BEGIN
    CREATE TYPE public.grn_reject_disposition AS ENUM ('RTV', 'SCRAP', 'DAMAGE', 'SHRINK');
EXCEPTION
    WHEN duplicate_object THEN NULL;
END $$;

ALTER TABLE public.goods_receipt_items
    ADD COLUMN IF NOT EXISTS reject_disposition public.grn_reject_disposition,
...
-- --------------------------------------------------------------------
-- goods_receipt_items_post_stocking: per-line route_to_qc + accepted qty
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- Rejected quantity disposition posting
-- --------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.goods_receipt_items_post_reject()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, private
...
DECLARE
    v_gr public.goods_receipts%ROWTYPE;
    v_reject_qty NUMERIC(15, 4);
    v_scrap_loc UUID;
    v_loss_loc UUID;
    v_target_loc UUID;
    v_ref TEXT;
BEGIN
    v_reject_qty := GREATEST(COALESCE(NEW.quantity_rejected, 0), 0);
    IF v_reject_qty <= 0 OR NEW.is_promotional THEN
        RETURN NEW;
    END IF;
    ...
    v_ref := v_gr.voucher_number || '|REJECT-' || COALESCE(NEW.reject_disposition::TEXT, 'SCRAP');

    IF NEW.reject_disposition = 'SHRINK'::public.grn_reject_disposition THEN
        SELECT id INTO v_loss_loc FROM public.tenant_locations
        WHERE tenant_id = NEW.tenant_id AND code = '_SYSTEM_LOGISTICAL_LOSS' LIMIT 1;
        v_target_loc := COALESCE(v_loss_loc, v_gr.destination_location_id);
    ELSE
        SELECT id INTO v_scrap_loc FROM public.tenant_locations
        WHERE tenant_id = NEW.tenant_id AND code = '_SYSTEM_SCRAP_QUARANTINE' LIMIT 1;
        v_target_loc := COALESCE(v_scrap_loc, v_gr.destination_location_id);
    END IF;
    ...
    VALUES (
        NEW.tenant_id, NEW.item_id, NEW.variant_id, v_target_loc,
        'PURCHASE_RECEIPT', v_reject_qty, NEW.raw_unit_cost,
        v_ref, v_gr.created_by
    );
...
DROP TRIGGER IF EXISTS goods_receipt_items_post_reject_trg ON public.goods_receipt_items;
CREATE TRIGGER goods_receipt_items_post_reject_trg
    AFTER INSERT ON public.goods_receipt_items
    FOR EACH ROW
    EXECUTE FUNCTION public.goods_receipt_items_post_reject();

-- --------------------------------------------------------------------
-- Partial QC line release
-- --------------------------------------------------------------------
...
BEGIN
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'QC hold row not found for line'; END IF;
    ...
    IF v_release > 0 THEN
        ...
        SET quantity_on_hand = quantity_on_hand - v_release, updated_at = NOW()
        WHERE id = v_row.id;
    END IF;
    ...
    DELETE FROM public.qc_inventory_balances
    WHERE id = v_row.id AND quantity_on_hand <= 0.0001;

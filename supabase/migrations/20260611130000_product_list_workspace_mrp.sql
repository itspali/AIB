-- Append MRP from items.custom_fields to list workspace views (append-only column order).
...
SELECT
    ...
    COALESCE(
        NULLIF(TRIM(i.custom_fields ->> 'mrp'), '')::NUMERIC,
        NULLIF(TRIM(i.custom_fields ->> 'max_retail_price'), '')::NUMERIC,
        NULLIF(TRIM(i.custom_fields ->> 'maximum_retail_price'), '')::NUMERIC
    ) AS mrp
FROM public.items i
